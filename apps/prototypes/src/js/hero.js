...
gsap.set(ambianceVideo, { cursor: 'pointer' });
gsap.set(ambianceVideo, { height: '50vh' });
...
tlMaster.fromTo(
  desktopvideoMarker,
  { display: 'none' }, // Start from hidden state
  {
    display: 'block',
    duration: 1,
    // startAtTime: -1
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Tab' && event.shiftKey) {
    for (const bouton of boutons) {
      const contenuId = bouton.dataset.controls;
      const contenu = document.querySelector(`#${contenuId}`);
      const firstFocusableElement = contenu.querySelector(
