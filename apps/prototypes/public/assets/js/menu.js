    ...
    case 'mainMenuButton':
      {
        ...
        if (!mainMenuButton.attributes['data-expanded'] || mainMenuButton.attributes['data-expanded'].value === 'true') {
          ...
          setTimeout(() => {
            mainMenuButton.focus();
            if (/iPad|iPhone|iPod/.test(navigator.userAgent) || navigator.platform.toUpperCase().includes('MAC')) {
              console.log('apple');
              setTimeout(() => {
                mainMenuButton.blur();
              }, '500');
...
// handleMediaQueryChange(mediaQuery);//# sourceMappingURL=menu.js.map
